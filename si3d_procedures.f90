...
!------------------------------------------------------------------------
   ...
   READ (UNIT=i5,FMT='(///(14X,G20.2))',IOSTAT=ios) itrap,niter,ismooth, &
       ...
       & ibc,isal, itrsca, cd, ifsurfbc, dtsurfbc, cw, wa, phi, idbg
...
!-------------------------------------------------------------------------
   ...
   IF (ibathyf == 1) THEN ! Stockton
     ...
     !WRITE (UNIT=fmt, FMT='("(5X,", I4, "G5.1)")') ncols ! idt real
...
!-----------------------------------------------------------------------
   ...
   CASE (1)
      ...
      haxpp(:,lm1) = 0.0; haypp(:,lm1) = 0.0
!$OMP PARALLEL DO private(i,j,k,k1x,kmx)
      DO l = 1, lm
        haypp(:,l) = 0.0; haxpp(:,l) = 0.0; th(:,l) = 0.0; th1(:,l) = 0.0
 ...
 !$OMP END PARALLEL DO
      !......Calulate the explicit terms by sweeping over interior u-pts.....
 !$OMP PARALLEL DO private(i,j,k,k1x,kmx,corx,uE,uW,vN,vS,wU,wD,scE,scW,scN,scS,advx,hdx)
      DO l = 1, lm
         ex(:,l) = 0.0
         ...
         DO k = k1x,kmx
            ...
            SELECT CASE ( itrmom)
            CASE (1)
              scE = (up(k,lEC(l))*hup(k,lEC(l))+  &
                     up(k,    l )*hup(k,    l))/  &
                    (hup(k,lEC(l))+hup(k,l))
              scW = (up(k,lWC(l))*hup(k,lWC(l))+  &
                     up(k,    l )*hup(k,    l))/  &
                    (hup(k,lWC(l))+hup(k,l))
              scN = (up(k,lNC(l))*hup(k,lNC(l))+  &
                     up(k,    l )*hup(k,    l))/  &
                    (hup(k,lNC(l))+hup(k,l))
              scS = (up(k,lSC(l))*hup(k,lSC(l))+  &
                     up(k,    l )*hup(k,    l))/  &
                    (hup(k,lSC(l))+hup(k,l))
            ...
            ! Vertical advection - Upwind only for the first
            ! interface below the free surface; Centered diff.
            ! otherwise
            IF (k > k1x + 1) THEN
            ...
            !.....Final explicit term.....
            ex(k,l) = uhpp(k,l) - twodt1*(advx*iadv-corx-hdx*ihd)
...
!$OMP END PARALLEL DO
   ...
   CASE (2)
      ...
      haxpp(:,lm1) = 0.0; haypp(:,lm1) = 0.0
!$OMP PARALLEL DO private(i,j,k,k1y,kmy)
      DO l = 1, lm
         haxpp(:,l) = 0.0; haypp(:,l) = 0.0; th(:,l) = 0.0; th1(:,l) = 0.0
...
!$OMP END PARALLEL DO
      !......Calulate the explicit terms by sweeping over interior u-pts.....
!$OMP PARALLEL DO private(i,j,k,k1y,kmy,cory,uE,uW,vN,vS,wU,wD,scE,scW,scN,scS,advy,hdy)
      DO l = 1, lm
         ex(:,l) = 0.0
         ...
         DO k = k1y,kmy
            ...
            SELECT CASE ( itrmom)
            CASE (1)
              scE = (vp(k,lEC(l))*hvp(k,lEC(l))+  &
                     vp(k,    l )*hvp(k,    l))/  &
                    (hvp(k,lEC(l))+hvp(k,l))
              scW = (vp(k,lWC(l))*hvp(k,lWC(l))+  &
                     vp(k,    l )*hvp(k,    l))/  &
                    (hvp(k,lWC(l))+hvp(k,l))
              scN = (vp(k,lNC(l))*hvp(k,lNC(l))+  &
                     vp(k,    l )*hvp(k,    l))/  &
                    (hvp(k,lNC(l))+hvp(k,l))
              scS = (vp(k,lSC(l))*hvp(k,lSC(l))+  &
                     vp(k,    l )*hvp(k,    l))/  &
                    (hvp(k,lSC(l))+hvp(k,l))
            ...
            ! otherwise
            IF (k > k1y + 1) THEN
              advy=advy+(wU*(th (k  ,l)* vp(k  ,l)  +        &
                             th1(k  ,l)* vp(k-1,l)) -        &
                         wD*(th (k+1,l)* vp(k+1,l)  +        &
                             th1(k+1,l)* vp(k  ,l)) ) / 2.
            ELSEIF (k == k1y) THEN
              advy=advy-((wD+ABS(wD)) * vpp(k+1,l) +         &
                         (wD-ABS(wD)) * vpp(k  ,l)) / 4.
            ELSE
            ...
            !.....Final explicit term.....
            ex(k,l) = vhpp(k,l) - twodt1*(advy*iadv+cory-hdy*ihd)
...
!-----------------------------------------------------------------------
   ...
   INTEGER :: i, j, k, l, k1s, kms, kt, nwlayers, inn, kk, noc
...
!-----------------------------------------------------------------------
   ...
   DO nn = 1, nnodes
     ...
     ELSE
        ...
        DO k  = k1, kmzij
          ...
          DO l = 1,lm
            ...
            IF (ntr>0) THEN
              DO it = 1, ntr
                trout(k,it) = trout(k,it)+tracer(k,l,it)
              ENDDO
            ENDIF
          ENDDO
          IF (nk > 0) THEN
     ...
     IF (ntr <= 0) THEN
       WRITE (UNIT=i60, FMT=4) thrs, n, s(i,j), (zlevel(k+1),     &
           & uout (k), vout(k), wout(k), Avout(k), Dvout(k),      &
           & scout(k), k =k1,kmzij)
     ELSE
       WRITE (UNIT=i60, FMT=5) thrs, n, s(i,j), (zlevel(k+1) ,     &
            & uout (k), vout(k) , wout(k), Avout(k), Dvout(k),     &
            & scout(k  ),                                          &
            & trout(k,1),                                          &
            & trout(k,2),                                          &
   ...
   4 FORMAT(1X,F10.3,I10,2PF9.2,0PF9.2,2(2PF10.2),2PF9.4,2(4PF15.7),   0PE15.7 / &
	                & ( 30X,0PF9.2,2(2PF10.2),2PF9.4,2(4PF15.7),   0PE15.7 ))
   5 FORMAT(1X,F10.3,I10,2PF9.2,0PF9.2,2(2PF10.2),2PF9.4,2(4PF15.7),16(0PF15.7)/ &
...
!-----------------------------------------------------------------------
   ...
   ! ... Local variables
   INTEGER :: i, j, k, l, k1s, kms, gamma1, istat
   REAL    :: vel, ratio, C_f, delz, twodt1, hd, vsettle
   REAL, DIMENSION (4          ) :: ss
   REAL, DIMENSION(:,:), POINTER :: hdxpp, hdypp
   ...
   DO l = 1, lm;
     ...
     DO k = k1s, kms;
       ...
       IF (hp(k-1,l)> ZERO) THEN
         ...
         vel  = wp(k,l) - vsettle; IF (k == k1s) vel = 0.0;
...
!-----------------------------------------------------------------------
   ...
   INTEGER :: nn, inn, i, j, k, kk, l, k1s, kms, nwl, ktop, ksrc, plmdim, itr
